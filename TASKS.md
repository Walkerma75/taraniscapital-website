...
- [ ] **Enable CloudTrail on the `TaranisCapital` account** — no trail currently exists. Single-region management-events trail is free. Configure: trail name `taranis-capital-mgmt`, S3 destination bucket (new, separate from website buckets), log file validation on, CloudWatch Logs off initially. Audit trail is cheap insurance even if never actively read.
...
- [ ] **Run a restore-from-nothing drill** — Tier 2 Handover checklist requires it. Clone repo to a scratch directory, point a scratch S3 bucket + distribution at it, confirm the pipeline rebuilds the site end-to-end. Dry-run only, don't touch production buckets.
